...
@click.option("--port", type=int, default=6060, help="Port to bind to")
@click.option(
    "--auto-refresh",
    is_flag=True,
    help="Enable auto-refresh of service data",
)
@click.pass_context
def serve(ctx: click.Context, host: str, port: int, auto_refresh: bool) -> None:
    """Serve the portal via HTTP server."""
    config = ctx.obj["config"]
    ...
    try:
        ...
        async def handle_portal(request: Any) -> Any:
            ...
            try:
                ...
                portal_html = portal_gen.generate_portal(result)

                # Read and return HTML
                html_content = Path(portal_html).read_text()
                return web.Response(text=html_content, content_type="text/html")

            except Exception as e:
                logger.error(f"Failed to generate portal: {e}")
                return web.Response(text=f"Error: {e}", status=500)

        async def handle_json(request: Any) -> Any:
            ...
            try:
                ...
                json_content = Path(json_file).read_text()
        ...
        app.router.add_get("/api/services", handle_json)

        # Start server
        web.run_app(app, host=host, port=port)

    except ImportError:
        logger.error("aiohttp is required for serve command")
        sys.exit(1)
    except Exception as e:
