...
draw.ellipse((inner_pos, inner_pos, inner_pos + inner_size - 1, inner_pos + inner_size - 1), fill=inner_color)
...
for pos in bolt_positions:
    draw.ellipse((pos[0] - bolt_radius, pos[1] - bolt_radius, 
                  pos[0] + bolt_radius, pos[1] + bolt_radius), fill=bolt_color)

# Save as PNG first
png_path = os.path.join(os.path.dirname(__file__), 'porthole.png')
image.save(png_path)
